...
test_kill_general_bracket_nested =
  assert "kill/bracket/general/nested" do
    ref <- newRef []
    let
      action s = do
        _ ← modifyRef ref (_ <> [ s ])
...
test_kill_finalizer_catch =
  assert "kill/finalizer/catch" do
    ref ← newRef ""
    fiber ←
      forkAff
        $ bracket
...
test_kill_finalizer_bracket =
  assert "kill/finalizer/bracket" do
    ref ← newRef ""
    fiber ←
      forkAff
        $ bracket
...
test_parallel_throw =
  assert "parallel/throw"
    $ withTimeout (Milliseconds 100.0) do
        ref ← newRef ""
        let
          action n s = do
...
test_parallel_alt =
  assert "parallel/alt" do
    ...
    f1 ←
      forkAff $ sequential
        $ parallel (action 10.0 "foo")
        <|> parallel (action 5.0 "bar")
    delay (Milliseconds 10.0)
    r1 ← readRef ref
    r2 ← joinFiber f1
    pure (r1 == "bar" && r2 == "bar")
...
test_kill_parallel_alt_finalizer =
  assert "kill/parallel/alt/finalizer" do
    ref ← newRef ""
    f1 ←
      forkAff $ sequential
        $ parallel (delay (Milliseconds 10.0))
        <|> parallel do
            bracket
              (pure unit)
              ( \_ → do
...
test_efffn =
  assert "efffn" do
    ...
    let
      effectDelay ms =
        AC.fromEffectFnAff $ AC.EffectFnAff
          $ AC.mkEffectFn2 \ke kc → do
              fiber ← runAff (either (AC.runEffectFn1 ke) (AC.runEffectFn1 kc)) (delay ms)
              pure $ AC.EffectFnCanceler
                $ AC.mkEffectFn3 \e cke ckc → do
...
test_regression_return_fork =
  assert "regression/return-fork" do
    bracket
      (forkAff (pure unit))
      (const (pure unit))
      (const (pure true))
...
test_regression_par_apply_async_canceler =
  assert "regression/par-apply-async-canceler" do
    ref ← newRef ""
    let
      action1 =
        makeAff \_ →
...
test_regression_kill_sync_async =
  assert "regression/kill-sync-async" do
    ref ← newRef ""
...
test_regression_bracket_kill_mask =
  assert "regression/kill-bracket-mask" do
    ref ← newRef ""
    let
      action s = do
        _ <- modifyRef ref (_ <> s)
...
test_regression_kill_empty_supervisor =
  assert "regression/kill-empty-supervisor" do
